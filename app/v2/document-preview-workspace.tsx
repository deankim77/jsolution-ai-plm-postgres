...
function DocumentWorkspaceBridge(props:any){
  ...
  const autoConvertPendingDwgs=async(projectId?:string)=>{
    if(!projectId)return;
    try{
      const response=await fetch(`/api/projects/${projectId}/dwg-auto-convert`,{method:"POST"});
      const result=await response.json().catch(()=>null) as {converted?:number}|null;
      if(response.ok&&Number(result?.converted||0)>0)setRefreshKey(value=>value+1);
    }catch{}
  };
  ...
  useEffect(()=>{
    const refresh=(event:Event)=>{
      ...
      window.setTimeout(()=>void autoConvertPendingDwgs(projectId),350);
    };
