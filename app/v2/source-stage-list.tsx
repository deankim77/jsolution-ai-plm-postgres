...
export function SourceStageList({kind,rows,onOpen}:{kind:"ECR"|"QUALITY";rows:SourceRow[];onOpen:(workflowId:string)=>void}){
  ...
  const targetText=(row:SourceRow)=>row.partNumber?`${row.partNumber} · ${row.partName||"품명 미지정"}${row.partRevision?` · Rev.${row.partRevision}`:""}`:(row.changeTarget||"PART 미지정");
  const contextFor=(row:SourceRow)=>{const status=row.workflowStatus||row.status,number=ecr?row.ecrNumber:row.qualityNumber,type=ecr?(codeLabel("DESIGN_CHANGE_TYPE",row.changeType)||row.changeType||"유형 미지정"):(codeLabel("QUALITY_ISSUE_TYPE",row.issueType)||row.issueType||"유형 미지정"),severity=ecr?(row.priority?`우선순위 ${priorityLabels[row.priority]||row.priority}`:"우선순위 미지정"):(severityLabels[row.severity||""]||"심각도 미지정");return {id:row.id,kind:ecr?"설계변경":"품질관리",title:`${number?`[${number}] `:""}${row.title}`,meta:`${row.projectName} · 대상 PART ${targetText(row)} · 현재 단계 ${row.currentStepName||"작성"} · 처리자 ${row.currentAssigneeName||"미배정"} · ${type} · ${severity} · 기한 ${row.dueDate||"미정"} · ${statusLabels[status]||status}`}};
  ...
  return <div className="wv2-workflow-table stage-table source-stage-table">
    ...
    {rows.map(row=>{const flow=String(row.stepFlow||"").split("~").filter(Boolean).map(value=>{const [name,status]=value.split("|");return {name,status}});const status=row.workflowStatus||row.status,item=contextFor(row),checked=selection.has(item);return <button key={row.id} disabled={!row.workflowId} onClick={()=>row.workflowId&&onOpen(row.workflowId)}>
      ...
      <span>{ecr?<><b>{codeLabel("DESIGN_CHANGE_TYPE",row.changeType)||(loading?"유형 확인 중":"유형 미지정")}</b><small>{targetText(row)}{row.drawingCode?` · 관련 도면 ${row.drawingCode}`:""}{row.priority?` · ${priorityLabels[row.priority]||row.priority}`:""}</small></>:<><b>{codeLabel("QUALITY_ISSUE_TYPE",row.issueType)||(loading?"유형 확인 중":"유형 미지정")}</b><small>{targetText(row)} · {severityLabels[row.severity||""]||"심각도 미지정"}</small></>}</span>
