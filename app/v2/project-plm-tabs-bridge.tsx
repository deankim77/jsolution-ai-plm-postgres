...
function DrawingList({rows,versions,taskMap,onOpen}:{rows:Deliverable[];versions:Version[];taskMap:Map<string,Task>;onOpen:(item:Deliverable)=>void}){return <><header className="project-plm-summary"><div><b>프로젝트 도면</b><span>현재 프로젝트와 연결된 도면 및 연결 Task를 표시합니다.</span></div><div><strong>{rows.length}</strong><span>도면</span><strong>{rows.filter(item=>registered(item,versions)).length}</strong><span>등록</span></div></header><div className="project-plm-table drawings"><div className="head"><span>도면번호</span><span>도면명</span><span>구분</span><span>WBS / Task</span><span>상태</span><span>Rev</span></div>{rows.map(item=>{const task=item.taskId?taskMap.get(item.taskId):undefined;return <button key={item.id} onClick={()=>onOpen(item)} disabled={!item.taskId}><span><b>{item.drawingCode||item.internalDrawingNumber||"자동부여"}</b></span><span className="name"><b>{item.name}</b><small>{item.internalDrawingNumber||""}</small></span><span>{item.drawingType||"도면"}</span><span>{task?<><b>{task.wbsCode}</b><small>{task.name}</small></>:"프로젝트 공통"}</span><span>{registered(item,versions)?statusLabel(item.status):"미등록"}</span><span>{item.version||((item.revisionCount||0)>0?`Rev.${String(item.revisionCount).padStart(2,"0")}`:"-")}</span></button>})}{!rows.length&&<div className="empty">프로젝트에 연결된 도면이 없습니다.</div>}</div></>}
