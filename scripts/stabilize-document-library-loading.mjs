...
async function patchDocumentWorkspace(){
  ...
  else if(!source.includes(newLoad))throw new Error("document workspace load block not found");
  ...
  else if(!source.includes(newRender))throw new Error("document workspace loading render block not found");

  if(source!==before)await writeFile(path,source,"utf8");
}
...
async function patchMyWorkTiming(){
  ...
  source=source.replace(/  const totalStarted=Date\.now\(\);\r?\n  const timings:Record<string,number>=\{\};\r?\n  const timed=async<T>\(name:string,work:Promise<T>\)=>\{const started=Date\.now\(\);try\{return await work\}finally\{timings\[name\]=Date\.now\(\)-started\}\};\r?\n  const auth=await timed\("auth",getChatGPTUser\(\)\);\r?\n  const db=await timed\("db",runtimeDb\(\)\);\r?\n  let context;try\{context=await timed\("context",resolveRequestContext\(request,db,\{email:auth\?\.email\}\)\)\}catch\(reason\)\{return contextErrorResponse\(reason\)\?\?Response\.json\(\{error:"로그인이 필요합니다\."\},\{status:401\}\)\}/,
'  const auth=await getChatGPTUser();\n  const db=await runtimeDb();\n  let context;try{context=await resolveRequestContext(request,db,{email:auth?.email})}catch(reason){return contextErrorResponse(reason)??Response.json({error:"로그인이 필요합니다."},{status:401})}');
  ...
  source=source.replace(/    timed\("task",taskStatement\.all\(\)\),\r?\n    timed\("count",countStatement\.first<SummaryRow>\(\)\),\r?\n    includeSummary\?timed\("summary",summaryStatement\.first<SummaryRow>\(\)\):Promise\.resolve\(null\),\r?\n    timed\("projects",projectsStatement\.all<ProjectOption>\(\)\),/,
'    taskStatement.all(),\n    countStatement.first<SummaryRow>(),\n    includeSummary?summaryStatement.first<SummaryRow>():Promise.resolve(null),\n    projectsStatement.all<ProjectOption>(),');
  ...
  source=source.replace(/  timings\.total=Date\.now\(\)-totalStarted;\r?\n  const serverTiming=Object\.entries\(timings\)\.map\(\(\[name,duration\]\)=>`\$\{name\};dur=\$\{duration\}`\)\.join\(", "\);\r?\n  console\.info\("\[MY_WORK_TIMING\]",JSON\.stringify\(\{scope,includeSummary,limit,offset,\.\.\.timings\}\)\);\r?\n\r?\n/,"");

  source=source.replace(/,\s*\{headers:\{"server-timing":serverTiming\}\}/g,"");

  if(source.includes("[MY_WORK_TIMING]")||source.includes("const totalStarted=Date.now()")||source.includes("timed(\"task\"")){
    throw new Error("MY_WORK timing debug removal incomplete");
  }
  if(source!==before)await writeFile(path,source,"utf8");
}
